    ...
    with st.expander("음성비서 프로그램에 관하여", expanded=True):
        st.write(
            """
                ...
                - STT(Speech-To-Text)는 OpenAI의 Whisper AI툴 활용했습니다.
                - 답변은 OpenAI의 GPT 모댈을 활용했습니다.
    ...
    with col2:
        ...
        if (audio.duration_seconds > 0) and (st.session_state["check_reset"]==False):
            ...
            for sender, time, message in st.session_state["chat"]:
                if sender == "user":
                    st.write(f'<div style="display: flex; align-items:center;" ><div style="background-color: #007AFF; color :white;border-radius:12px; padding:8px 12px;marginright:8px; ">{message}</div><div style="font-size :0.8rem;color :gray ;">{time}</div></div>'
                             , unsafe_allow_html = True)
                    st.write("")
                else:
                    st.write(f'<div style="display:flex; align-items: center; justifycontent:flex-end;"><div style="background-color: lightgray; border-radius:12px; padding :8px12px; margi n-left :8px;">{message}</di v><div style="font-size:0 .8rem; color:gray;">{time}</div></div>'
                             , unsafe_allow_html = True)
                    st.write("")
